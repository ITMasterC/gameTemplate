...
cc.Class({
    ...
    initLevelItem(type = 2) {
        if (cc.sys.platform != cc.sys.WECHAT_GAME) {
            ...
            var item = null;
            var itemCount = 0;
            let randomArr = comFun.randomArr(10, 10);
            if (this.itemNodeArr.length == 0) {
                ...
                console.log('-----------this.itemNodeArr.length', this.itemNodeArr.length);
                ...
                this.scrollViewContent.width = item.height * itemCount + (itemCount) * this.scrollViewContent.getComponent(cc.Layout).spacingX;
            }
            return;
        }
        this.addNumber = 0.8;
    ...
    update(dt) {
        if (this.scrollViewContent.x >= -this.scrollViewContent.parent.width / 2) {
            this.addNumber = -0.8;
        } else if (this.scrollViewContent.x <= this.scrollViewContent.parent.width / 2 - this.scrollViewContent.width) {
            this.addNumber = 0.8;
        }
        this.scrollViewContent.x += this.addNumber;
    },
});